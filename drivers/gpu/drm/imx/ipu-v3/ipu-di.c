...
static unsigned long ipu_di_clk_get_parent_rate(struct ipu_di *di)
{
	struct clk *parent;
...
unsigned long ipu_di_clk_get_rate(struct ipu_di *di)
{
	unsigned long inrate = ipu_di_clk_get_parent_rate(di);
...
long ipu_di_clk_round_rate(struct ipu_di *di, unsigned long rate)
{
	unsigned long inrate = ipu_di_clk_get_parent_rate(di);
	unsigned long outrate;
	int div;
	...
	if (di->clk_ext) {
		...
		if (di->pll_clk)
			return clk_round_rate(di->pll_clk, rate);
		return 0;
	}
...
EXPORT_SYMBOL_GPL(ipu_di_clk_set_rate);
		...
		u32 module, struct clk *ipu_clk)
{
	...
	di->clk_ext = ipu->di[id].clk_ext;
	di->clk_sync = ipu->di[id].clk_sync;
	...
	di->hsp_clk = clk_get(dev, "bus");
	if (IS_ERR(di->hsp_clk)) {
		ret = PTR_ERR(di->hsp_clk);
		goto err_clk1;
	}

	di->pll_clk = clk_get(dev, "di0-pll");
	if (IS_ERR(di->pll_clk))
		di->pll_clk = NULL;
